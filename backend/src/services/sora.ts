...
export class SoraService extends AIService {
  ...
  async processImage(imagePath: string, options: ProcessImageOptions): Promise<ProcessImageResult> {
    try {
      ...
      // Sora processes fewer variants but with higher creativity
      const maxVariants = Math.min(promptTemplates.length, 2); // Limit to 2 variants for Sora
      
      for (let i = 0; i < maxVariants; i++) {
        const template = promptTemplates[i];
        try {
          const processedImage = await this.callSoraAPI(base64Data, mimeType, template.base);
          ...
          console.error(`Sora variant ${i + 1} generation failed:`, error);
        }
      }

      if (variants.length === 0) {
        throw new Error('No variants were successfully generated by Sora');
      }
  ...
  private buildPromptTemplates(options: ProcessImageOptions): PromptTemplate[] {
    ...
    // Sora-specific prompts with creative and artistic focus
    ...
    // Enhance prompts for Sora's creative capabilities
    prompts = prompts.map(prompt => 
      `As a creative visual artist with expertise in innovative image transformation, ${prompt} 
      Apply artistic vision and creative interpretation. Experiment with unique visual styles, 
      artistic filters, and innovative compositions while maintaining the core subject matter.
      Focus on creating visually striking and artistically enhanced results.`
    );
  ...
  getModelInfo(): AIModelInfo {
    return {
      ...
      name: 'Sora Image',
      description: '创新性图像处理模型，专注于艺术创作和实验性视觉效果',
      capabilities: ['艺术创作', '创意滤镜', '风格转换', '实验性效果'],
      speed: 'slow',
      quality: 'premium',
      recommended: ['艺术创作', '创意设计', '风格实验', '独特视觉效果']
    };
  }
